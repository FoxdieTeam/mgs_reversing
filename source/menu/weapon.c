...
{
    if (((GM_ItemTypes[GM_Item + 1] & 1) && (GM_WeaponTypes[weaponId + 1] & 0x200)))
    {
        return TRUE;
    }
...
char *wpn_descriptions_8009E5CC[] = {
    /* WP_Socom */
    (char[]){0xB0,0x14,0x82,0x1D,0xD0,0x06,0x82,0x13,0x82,0x40,0x82,0x34,0x82,0x19,0x82,0x28,
    ...
    /* WP_Stinger */
    (char[]){0xB0,0x14,0x82,0x19,0x82,0x26,0xC2,0x03,0x82,0x53,0x82,0x0C,0xD0,0x06,0xC0,0x7F,
    ...
    (char[]){0xB0,0x14,0x43,0x80,0x34,0x90,0xA2,0x90,0x85,0xD0,0x15,0x80,0x7C,0x82,0x37,0x82,
             ...
             0x82,0xD0,0x03,0x00},
    /* WP_StunGrenade */
    (char[]){0xB0,0x14,0x82,0x19,0x82,0x1F,0x82,0x53,0xC0,0x7F,0x82,0x10,0x82,0x4C,0x82,0x2D,
             ...
             0xD0,0x03,0x00,0x00},
    /* WP_ChaffGrenade */
    (char[]){0xB0,0x14,0x82,0x21,0xC2,0x43,0x82,0x35,0xC0,0x7F,0x82,0x10,0x82,0x4C,0x82,0x2D,
             ...
             0xD0,0x03,0x00,0x00},
    /* WP_Rifle */
    (char[]){0xB0,0x14,0x80,0x50,0x80,0x53,0x80,0x47,0x80,0x31,0xD0,0x15,0x80,0x7C,0x90,0xAA,
...
void menu_weapon_printDescription_8003E030(int wpn_id)
{
    ...
    else if (GM_GameLevel == GM_LEVEL_VERYEASY && wpn_id == WP_Famas)
    {
...
void menu_weapon_init_helper_8003E0E8(MenuWork *work, u_long *ot, int off_x, int off_y, PANEL *pPanel)
{
    ...
    if (weaponIdx >= 0)
    {
        ...
        {
            pSubCnt2 = (GM_Weapon == WP_Famas ? 3 : 0);
...
void menu_weapon_update_helper2_helper2_8003E3B0(MenuWork *work)
{
    ...
    {
        GM_Weapon = work->field_1F0_menu_weapon.field_0_current.field_0_id;
        GM_WeaponChanged = 1;
        if (GM_Weapon >= 0)
        {
            sub_8003CFE0(
        ...
        work->field_1F0_menu_weapon.field_11 = varbuf[14]; // GM_Weapon would not match...
    }
    else
    {
        if (id != WP_None)
...
void menu_weapon_update_helper2_8003E674(MenuWork *work, u_long *ot)
{
    ...
    case 0:
        ...
        else
        {
            ...
            if (menu_weapon_isWeaponDisabled_8003DF30(GM_Weapon))
            {
                last_id = GM_Weapon;
                GM_Weapon = WP_None;
                GM_WeaponChanged = 1;
            ...
            if (GM_Weapon != work->field_1F0_menu_weapon.field_0_current.field_0_id)
            {
                switched_weapon = 1;
                GM_WeaponChanged = 1;
                printf("HERE %d\n", work->field_1F0_menu_weapon.field_0_current.field_0_id);
                ...
                work->field_1F0_menu_weapon.field_0_current.field_0_id = GM_Weapon;
            }
            ...
            if (GM_Weapon >= 0)
            {
                ...
                {
                    sub_8003CFE0(menu_weapon_get_weapon_rpk_info_8003DED8(GM_Weapon), 1);
                    work->field_1F0_menu_weapon.field_11 = GM_Weapon;
                }

                work->field_1F0_menu_weapon.field_0_current.field_2_num = GM_Weapons[GM_Weapon];
                menu_sub_menu_update_8003DA0C(work, ot, &work->field_1F0_menu_weapon);
            }
        }
        break;
        ...
        if (!(GM_PlayerStatus & (PLAYER_MENU_DISABLE | PLAYER_WEAPON_DISABLE | PLAYER_PAD_OFF)))
        {
            ...
            else if ((!(GM_GameStatus & GAME_FLAG_BIT_19)) && (pPad->press & PAD_R1))
            {
                ...
                else if (!menu_weapon_isWeaponDisabled_8003DF30(work->field_1F0_menu_weapon.field_11))
                {
                    ...
                    {
                        GM_Weapon = selected_id;
                    }
                }

                if (weapon_id != GM_Weapon)
                {
                    GM_WeaponChanged = 1;
