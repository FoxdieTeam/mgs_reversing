...
extern unsigned short Rasen_Maps[];
...
void RasenElAct_800CC454(Work *work)
{
    ...
    GM_CurrentMap = Rasen_Maps[Rasen_MapSection];
...
int RasenElGetResources_800CCB9C(Work *work, int name, int map)
{
    ...
    work->hzd[0] = GM_GetMap(Rasen_Maps[0])->hzd;
    work->hzd[1] = GM_GetMap(Rasen_Maps[1])->hzd;
...
void s11c_800CDAB4(Work *work)
{
    ...
    prim = GM_MakePrim(DG_PRIM_ONESIDE | DG_PRIM_POLY_FT4, 3, work->f250, NULL);
    work->f24C = prim;
    ...
    prim->group_id = Rasen_Maps[0] | Rasen_Maps[1];
    prim->raise = 0;
