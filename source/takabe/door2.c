...
void s13a_door2_800DD274(POLY_FT4 *packs0, POLY_FT4 *packs1, int n_packs, DG_TEX *tex)
{
    ...
    {
        setPolyFT4(packs0);
        LSTORE((LLOAD(&packs0->r0) & RGBA_A_MASK) | COLOR_GRAY, &packs0->r0);

        setPolyFT4(packs1);
        LSTORE((LLOAD(&packs1->r0) & RGBA_A_MASK) | COLOR_GRAY, &packs1->r0);
...
int Door2GetResources_800DD7C8(Door2Work *work, int name, int map)
{
    ...
    prim1 = GM_MakePrim(DG_PRIM_POLY_FT4, 22, verts, NULL);
    work->prim1 = prim1;
    if (prim1 == NULL)
    {
        return -1;
    ...
    prim2 = GM_MakePrim(DG_PRIM_POLY_FT4, 22, verts, NULL);
    work->prim2 = prim2;
    if (prim2 == NULL)
    {
        return -1;
