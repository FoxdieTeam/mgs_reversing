...
static inline void SUB( DVECTOR *res, HZD_VEC *vec1, HZD_VEC *vec2 )
{
    res->vx = vec1->x - vec2->x;
    res->vy = vec1->z - vec2->z;
}

static inline long DOT( DVECTOR *vec1, DVECTOR *vec2 )
{
    TEMP->vy = vec2->vx;
    TEMP->vx = -vec2->vy;
    gte_ldsxy3( 0, *(int *)vec1, *(int *)TEMP );
    gte_nclip();
    gte_stopz( SIDE );
    return *SIDE;
}
...
static int SlopeFloorLevel( HZD_FLR *flr )
{
    ...
    SUB( P1_FROM, FROM, &flr->p1 );
    NORMAL->vx = flr->p1.h;
    NORMAL->vy = flr->p2.h;
    pf_n = DOT( P1_FROM, NORMAL );
