...
static int PollMessages(Work *work)
{
    ...
    for (i = control->n_msg, msg = control->msg; i > 0; i--, msg++)
    {
            ...
            if (message_type == hash_enter)
            {
                ...
                if (++work->field_F2_door_counter > 0)
                {
                    ...
                    if (msg->message_len > 1 && work->field_F4_param_g_v > 0)
                    {
                        ...
                        if (work->field_F0 == SNAKE_NAME || (GM_PlayerMap & work->where))
                        {
                            GM_AddMap(work->field_F8_maps[0]);
                            GM_AddMap(work->field_F8_maps[1]);
                        }

                        if (work->field_F0 == SNAKE_NAME)
                        {
                            printf("Snake Door %X\n", work->where);
...
static int GetResources(Work *work, int name, int where)
{
    ...
    work->field_E6_param_w_v = work->object.objs->def[2].n_models; // is this correct?
