...
void Splash2Act_800DAF38( Work *work )
{
    ...
    if ( work->timer <= 0 )
    {
        Splash2ShadePacks_800DAF0C( work->prim->packs[0], 8, 0 );
        Splash2ShadePacks_800DAF0C( work->prim->packs[1], 8, 0 );
...
int SplashGetResources_800DB210( Work *work, MATRIX *matrix, int noripple, int r, int g, int b )
{
    ...
    prim = work->prim = GM_MakePrim( DG_PRIM_POLY_FT4, 8, work->vecs2, &rect_800C3670 );
    if ( prim == NULL )
    {
        return -1;
    }
