...
int wolf2_GetResources2(Wolf2Work *work, int name, int where)
{
    ...
    work->lsight_prim = prim = GM_MakePrimChanl(DG_PRIM_LINE_FT2, 8, work->lsight_verts, NULL, 0);
    if (!prim)
    {
        return -1;
    }
    ...
    work->f7E0 = prim = GM_MakePrimChanl(DG_PRIM_OFFSET | DG_PRIM_TILE, 2, work->f7E4, &wolf2_800C3490, 0);
    if (!prim)
    {
        return -1;
    }
