...
typedef struct _Work
{
    ...
    MOTION_SEGMENT m_segs2[17];                              /* 0x464 */
    SVECTOR        rots[16];                                 /* 0x6E0 */
    int            field_760;                                /* 0x760 */
    int            field_764;                                /* 0x764 */
    char           pad_768[0x76C - 0x768];                   /* 0x768 */
} Work;
...
int s05a_800D4998(Work *work, int name)
{
    ...
    GM_ConfigMotionControl(&work->body, &work->m_ctrl, name, &work->m_segs1[0],
                           &work->m_segs2[0], &work->control, work->rots);
    work->field_760 = 0;
    work->field_764 = 0;
    return 0;
}
...
void *NewM1Throw(int name, int where)
{
    ...
    if (work != NULL)
    {
        GV_SetNamedActor(&work->actor, s05a_800D46A4, s05a_800D4968, s05a_dword_800E345C);
        if (s05a_800D4998(work, name) >= 0)
        {
            return work;
        }
