    ...
    else
    {
        ...
        if (work->f628 == 0)
        {
            if ((dist < 2000) || (GM_NoisePower > 0))
            {
                ...
                if ((work->f208 == 0) && (GM_NoisePower == 0) && (work->f62C < ++work->f200) && (dist > 1000))
                {
                    mov2 = control->mov;
                    Eventmouse_800C9308(work);
...
int EventMouseGetResources_800CA370(EventmouseWork *work, HZD_PTP *points, short n_points, int arg3, int arg4, int name, int map, int arg7, int arg8)
{
    ...
    prim = GM_MakePrimChanl(DG_PRIM_POLY_FT4, 1, work->prim_vecs, NULL, 0);
    work->prim = prim;
    if (prim != NULL)
    {
