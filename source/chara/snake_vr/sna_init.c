...
void sna_knock_80054D68(SnaInitWork *work, int time)
{
    ...
    if (time == 0)
    {
        ...
        if (sna_8004F628(work, &vec, -250, HZD_CHK_D_SEGMENT | HZD_CHK_F_SEGMENT, SEGMENT_ATR | HZD_SEG_NO_BEHIND, var_t0))
        {
...
static inline int GetResources(SnaInitWork *work, int scriptData, int scriptBinds)
{
    SVECTOR   shadow;
    SVECTOR   vec;
    int       attr;
...
void *NewSnake(int name, int where, int argc, char **argv)
{
    ...
    GV_SetNamedActor(&work->actor, Act, Die, "../snake_vr/sna_init.c");
