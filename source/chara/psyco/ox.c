...
static int CheckTurn( PSYOBJ *obj, int turn )
{
    obj->control->mov.vy = GV_NearExp8( obj->control->mov.vy, turn );
    if ( ABS( obj->control->mov.vy - turn ) <= 60 ) return 1;
    return 0;
}
