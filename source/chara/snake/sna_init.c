...
void sna_8004EC00(SnaInitWork *work)
{
    ...
    GM_ClearPlayerStatusFlag(PLAYER_STILL_WATCH);
}
...
void sna_8004EC8C(SnaInitWork *work)
{
    ...
    GM_ClearPlayerStatusFlag(PLAYER_WATCH);
}
...
void sub_8004F14C(SnaInitWork *work)
{
    ...
    {
        work->field_A56 = 0;
        GM_ClearPlayerStatusFlag(PLAYER_MENU_DISABLE | PLAYER_STILL_WATCH |
...
void sub_8004F454(SnaInitWork *work)
{
    ...
    GM_ClearPlayerStatusFlag(PLAYER_MOVE | PLAYER_ATTACK |
                             PLAYER_CHECK_WALL);
...
static inline int sna_update_life_helper_8004F6E8(int health, int item)
{
    ...
    if ((health == 0) &&
        ...
        !GM_FrozenItemsState)
    {
        temp = (GM_GameLevel > GM_LEVEL_EASY) ? 256 : 384;
        ...
        if (health > GM_VitalityMax)
        {
            health = GM_VitalityMax;
        }
...
void UpdateLife_8004F6E8(SnaInitWork *work)
{
    ...
    if (sna_check_flags1_8004E31C(work,SNA_FLAG1_UNK25) &&
        (GM_Vitality == work->field_89C_pTarget->vital))
    {
        return;
    }
    ...
    {
        work->field_89C_pTarget->damage /= 2;
        work->field_89C_pTarget->vital += work->field_89C_pTarget->damage;
        work->field_89C_pTarget->damage = 0;
    }

    health = work->field_89C_pTarget->vital + GM_Vitality - work->field_A22_snake_current_health;
    ...
    work->field_89C_pTarget->vital = health;
...
void GM_CheckShukanReverse(unsigned short *pInput)
{
    ...
    if ((GM_Configuration & GM_CONFIG_SHUKAN_REVERSE) != 0
...
int sub_8004FCB8(SnaInitWork *work, PUNCH *punch, int a_mode)
{
    ...
    GM_SetPowerTarget(target, POWER_CONST, a_mode, punch->life, punch->faint, &vec);
...
int sna_8004FDE8(SnaInitWork *work, Target_Data *pTargetData)
{
    ...
    GM_SetCaptureTarget(&work->field_8A0_target, pTargetData->field_18, pTargetData->field_1C, &work->field_8F4, &work->field_8FC);
...
void sna_8005027C(SnaInitWork *work, int time)
{
    ...
    if ((*work->field_918_n_bullets != 0) && (bakudan_count_8009F42C < 16))
    {
        ...
        if (work->field_A26_stance != STAND)
        {
...
static inline int sna_weapon_switching_helper_800511BC(SnaInitWork *work)
{
    ...
    if (!(work->field_920_tbl & 0x200) && GM_CheckPlayerStatusFlag(flag))
    {
        return 0;
    }
...
static inline int sna_weapon_switching_helper2_800511BC(SnaInitWork *work, int callback)
{
    ...
    pWeaponEntry = &gSnakeWeapons_8009EF3C[GM_Weapon + 1];
    pWeaponCreateFn = pWeaponEntry->mCreateActorFn;
    pWeaponActor = NULL;

    if (pWeaponCreateFn)
    {
        pWeaponActor = pWeaponCreateFn(&work->control, &work->body, 4, &work->field_914_trigger, PLAYER_SIDE);
    ...
    work->field_91C_weapon_idx = GM_Weapon;

    if (callback)
    {
        GM_CallSystemCallbackProc(3, GM_Weapon);
    }

    work->field_920_tbl = GM_WeaponTypes[GM_Weapon + 1];

    if (GM_Weapon >= 0)
    {
        work->field_918_n_bullets = &GM_Weapons[GM_Weapon];
    }
    ...
    work->actpack = &weapon_actions_8009ED8C[GM_Weapon];
...
void sna_weapon_switching_800511BC(SnaInitWork *work, int callback)
{
    ...
    if (GM_Weapon != work->field_91C_weapon_idx)
    {
        GM_WeaponChanged = 1;
    }
    ...
    if (!(work->field_920_tbl & 0x1000) && (GM_Weapon == work->field_91C_weapon_idx))
    {
        return;
    }
    ...
    if (temp_s1_2 & 0x200)
    {
        ...
        if (GM_FirstPerson != 0)
        {
            GM_SetPlayerStatusFlag(PLAYER_STILL_WATCH);
            sna_clear_flags2_8004E344(work, SNA_FLAG2_UNK6 | SNA_FLAG2_UNK5);
        }
    }
    else if (!GM_CheckPlayerStatusFlag(PLAYER_INTRUDE) && ((GM_FirstPerson == 0) || !(work->field_898_flags2 & 0x10)))
    {
        sna_8004EC00(work);
    }
...
static inline int sna_helper_800515BC(SnaInitWork *work)
{
    ...
    if ( (GM_Item == IT_Bandana) && (work->field_90C_pWeaponFn == &sna_80057A90) )
    {
        return 0;
    }
...
void sna_800515BC(SnaInitWork *work, int a2)
{
    ...
    if ( GM_Item == work->field_9A8_current_item )
    {
        return;
    }

    itemType = GM_ItemTypes[GM_Item + 1];
    ...
    ppfnEquip = &gSnakeEquips_8009EF8C[GM_Item];
    pItemActor = NULL;
    ...
    if ( ((itemType & 0x8000) != 0) && (*ppfnEquip != 0) )
    {
        ...
        if ( (GM_Item == 2) || (GM_Item == 3) || (GM_Item == 4) )
        {
    ...
    work->field_9A8_current_item = GM_Item;

    if ( a2 != 0 )
    {
        GM_CallSystemCallbackProc(4, GM_Item);
    }
    ...
    if ((itemType & 2) != 0)
    {
        ...
        GM_SetPlayerStatusFlag(PLAYER_WEAPON_DISABLE);
                ...
                ((GM_FirstPerson == 0) || ((work->field_898_flags2 & 0x10) == 0)) )
    {
        sna_8004EC00(work);
    }
...
void sna_80051DA0(SnaInitWork *work)
{
    ...
    {
        if (work->field_A26_stance == GROUND)
        {
...
void sna_anim_crouch_800527DC(SnaInitWork *work, int time)
{
    ...
    if (time == 0)
    {
        if (work->field_A26_stance != CROUCH)
        {
            sound = SE_CHANGE_STANCE;
            if (GM_CheckPlayerStatusFlag(PLAYER_IN_THE_WATER) != 0)
            {
    ...
    if ((time == 0 && work->field_A26_stance != STAND) ||
        (time == 4 && work->field_A26_stance == STAND))
    {
        GM_SetPlayerStatusFlag(PLAYER_SQUAT);
        work->field_A26_stance = CROUCH;
    }
}
...
void sna_anim_shot_flinch_800544E0(SnaInitWork *work, int time)
{
    ...
    if (time == 0)
    {
        ...
        if (work->field_89C_pTarget->vital < 1)
        {
            GM_SeSet2(0, 0x3f, SE_PLAYEROUT);
            sna_8004F8E4(work, work->field_A26_stance == GROUND ? 125 : 128);
        }
...
void sna_act_helper2_helper7_80054648(SnaInitWork *work, int time)
{
    if (time == 0)
    {
        ...
        if (work->field_89C_pTarget->vital <= 0)
        {
...
void sna_anim_knockdown_80054710(SnaInitWork *work, int time)
{
    ...
    if (time == 0)
    {
        ...
        {
            bVar1 = work->actpack->damage->field_3;
            work->field_A54.prone_bool_thing = 2;
        }

        SetAction_8004E22C(work, bVar1, 4);

        if (work->field_89C_pTarget->vital < 1 && GM_GameOverTimer == 0)
        {
            GM_SeSet2(0, 0x3f, SE_PLAYEROUT);
            GM_GameOverTimer = -1;
            sna_set_flags1_8004E2F4(work, SNA_FLAG1_UNK20);
...
void sna_anim_knockdown_shot_80054B50(SnaInitWork *work)
{
    ...
    if (work->field_89C_pTarget->vital <= 0 && !GM_GameOverTimer)
    {
        GM_SeSet2(0, 63, SE_PLAYEROUT);
...
void sna_anim_scope_80055334(SnaInitWork *param_1, int time)
{
    ...
    if (time == 0)
    {
        ...
        if (param_1->field_A26_stance == STAND)
        {
            action_flag = 0x87;
        }
        else
...
void sna_anim_dying_80055524(SnaInitWork *work, int time)
{
    ...
    if (time == 0)
    {
        ...
        if ((unk2->field_9EC_flags3 & 0x1000) == 0)
        {
            ...
            else if (work->field_91C_weapon_idx != -1 && GM_Weapon == WP_None)
            {
                sub_8004F14C(work);
            }
        }
...
void sna_act_unk_helper3_80055DD8(SnaInitWork *work, int time)
{
    ...
    case 6:
        ...
        else if (work->field_A26_stance == GROUND)
        {
            GM_SetPlayerStatusFlag(PLAYER_GROUND);
        }
...
void sna_80057378(SnaInitWork *work, int time)
{
    ...
    if ((stance == STAND && time == 6) || (stance == CROUCH && time == 14))
    {
...
void sna_anim_choke_rechoke_80058EF4(SnaInitWork *work, int time)
{
    ...
    if (time == 0)
    {
        ...
        work->field_904_frames_last_choke = 0;
        work->field_9CC_anim_update_fn_1p = sna_fn_nothing_80053B80;
        action_flag = actpack->attack->field_6;
        sna_8004E22C(work, actpack->attack->field_6, 4);
        work->field_8E8_pTarget->a_mode = action_flag;
        }
    }
}
...
void sna_anim_choke_drag_80059054(SnaInitWork *work, int time)
{
    ...
    {
        if (!((work->field_8E8_pTarget->vital > 0) &&
...
void sna_anim_choke_helper_8005951C(SnaInitWork *work, int time)
{
    ...
    if (((work->field_8E8_pTarget->vital < 1 || sinceLastChoke >= 90) ||
    ...
    else
    {
        ...
        if (pTarget->damaged & 0x6)
        {
            ...
            else
            {
                ...
                if ((pTarget->vital < 1) && ((GM_GameOverTimer > -2) && (GM_GameOverTimer < 1)))
                {
...
static inline int sna_init_main_logic_helper6_helper_800596FC(SnaInitWork *work, int flags)
{
    ...
    if ((GM_FirstPerson != 0) && !(work->field_898_flags2 & 0x10))
    {
        return 0;
    }
...
void sna_init_main_logic_800596FC(SnaInitWork *work)
{
    ...
    {
        var_v0_8 = GM_CheckPlayerStatusFlag(PLAYER_WATCH);
    }
    ...
    if ( GM_StreamStatus() == 2 )
    {
        ...
        sna_act_helper2_helper2_80033054(SNAKE_NAME, &work->adjust[6]);
...
static void Act(SnaInitWork *work)
{
    ...
    if ( (GM_FirstPerson != 0) && (GM_GameStatus & STATE_PADDEMO) )
    {
    ...
    /* しゃがみ天井突き抜け防止 */
    if ( (work->field_A26_stance != STAND) && (height >= 500) )
    {
    ...
    *work->enable_shadow = dword_800ABA1C == 0;
    ...
    pTarget2->force = work->control.step;
    ...
    {
        GM_SetPlayerStatusFlag(PLAYER_MOVE_WATCH);
    }
}
...
static inline int GetResources(SnaInitWork *work, int name, int where)
{
    ...
    GM_SetPowerTarget(trg, POWER_DECREASE, -1, GM_Vitality, 0, &DG_ZeroVector);
    GM_SetCaptureTarget(trg, 0, 0, &work->field_8F4, &work->field_8FC);
    GM_MoveTarget(trg, &work->control.mov);

    work->field_A22_snake_current_health = GM_Vitality;
