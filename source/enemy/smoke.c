...
int SmokeGetResources_800D2B0C(SmokeWork *work, int where)
{
    ...
    GM_CurrentMap = where;
    prim = GM_MakePrim(DG_PRIM_RECTANGLE | DG_PRIM_POLY_FT4, 1, &work->field_24, &smoke_rect);
    work->field_20 = prim;
    if (prim != NULL)
    {
        ...
        if (tex != 0)
        {
            s00a_smoke_800D2888(prim->packs[0], tex, 0x80);
            s00a_smoke_800D2888(prim->packs[1], tex, 0x80);
