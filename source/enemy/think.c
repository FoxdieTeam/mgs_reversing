...
int s00a_command_800CC344( WatcherWork *work )
{
    ...
    {
        work->pad.sound = SE_V_ENE_INMYWAY; // 敵兵「邪魔だ！」
    }
...
int s00a_command_800CC44C( WatcherWork *work )
{
    ...
    {
        work->pad.sound = SE_V_ENE_LOST; // 敵兵「どこ行った？」
    }
...
int Think3_NoiseModeWatch_800CC5C0( WatcherWork *work )
{
    ...
    if( work->count3 == 0)
    {
        if( EnemyCommand.mode  == TOP_COMM_TRAVEL )
        {
            ...
            {
                work->pad.sound = (  0x88   ) ; // SE_V_ENE_HUH
            }
        ...
        else
        {
            ...
            work->pad.sound = SE_V_ENE_THATSOUND; // 敵兵「今の音は！」
            ENE_PutMark( work ,BW_MARK );
            work->pad.dir = work->sn_dir;
        }
    }
...
int s00a_command_800CC83C( WatcherWork* work )
{
    if ( work->count3 == 0 )
    {
        if ( EnemyCommand.mode == TOP_COMM_TRAVEL )
        {
            work->pad.sound = SE_V_ENE_HUH; // 敵兵「ん？」
        }
        else
        {
            work->pad.sound = SE_V_ENE_WHATSTHAT; // 敵兵「何だ？」
        }
        ENE_PutMark( work, 5 );
        work->pad.dir = work->sn_dir;
    }
...
int s00a_command_800CCCAC( WatcherWork *work )
{
    ...
    if ( work->count3 == 30 )
    {
        ...
        {
            work->pad.sound = SE_V_ENE_GRENADE; // 敵兵「くらえ！」
        }
