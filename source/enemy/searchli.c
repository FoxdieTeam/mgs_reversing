...
void Searchli_800D80BC(SearchlightWork *work)
{
    ...
    if (GM_WhereList[0]->name != SNAKE_NAME)
    {
        return;
    }
...
int Searchli_800D9040(SearchlightWork *work)
{
    ...
    prim = GM_MakePrim(DG_PRIM_POLY_FT4, 9, work->fFC.pad2, NULL);
    work->fFC.prim = prim;
    if (prim == NULL)
    {
        return -1;
    ...
    Searchli_800D8DDC(prim->packs[0], tex, 2, 78, 78, 78, 3);
    Searchli_800D8DDC(prim->packs[1], tex, 2, 78, 78, 78, 3);
    return 0;
}
