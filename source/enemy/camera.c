...
int InitLed_800D6504(CameraWork *work)
{
    ...
    prim = GM_MakePrim(DG_PRIM_RECTANGLE | DG_PRIM_POLY_FT4, 1, &work->field_184, &camera_rect_800C3B68);
    work->field_194 = prim;
    if (prim != NULL)
    {
        prim->raise = 500;
        ...
        if (tex != NULL)
        {
            s01a_camera_800D648C(prim->packs[0], tex, 0x80);
            s01a_camera_800D648C(prim->packs[1], tex, 0x64);
            return 0;
        }
    }
    return -1;
