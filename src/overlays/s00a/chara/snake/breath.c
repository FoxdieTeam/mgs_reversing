...
extern const char aBreathC_800DFBD4[]; //breath.c
...
void *NewBreath_800C3A1C( int name, int where, int argc, char **argv )
{
    ...
    if ( work != NULL ) {
        /* ワークにコールバックを登録する */
        GV_SetNamedActor_8001514C( &( work->actor ), ( TActorFunction )BreathAct_800C38A0, ( TActorFunction )BreathDie_800C39AC, aBreathC_800DFBD4 ) ;
