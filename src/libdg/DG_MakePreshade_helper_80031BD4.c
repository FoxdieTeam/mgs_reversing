...
// TODO: This is something inside TARGET
typedef struct Target_Ukn
{
    SVECTOR vector[2];
    CVECTOR color[2];
} Target_Ukn;
...
CVECTOR * DG_MakePreshade_helper_80031BD4(DG_MDL *pMdl, CVECTOR *pRgbs, DG_OBJS *pObjs)
{
    ...
    for (faces = pMdl->numFaces_4 * 4; faces > 0; faces--)
    {
        ...
        if (index < 42)
        {
            unk = (Target_Ukn *)getScratchAddr(0) + index;
        }
        else
        {
            unk = (Target_Ukn *)&gTargets_800B64E0[62].field_10_size + index;
        }
