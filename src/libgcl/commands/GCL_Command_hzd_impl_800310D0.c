...
struct map_record *GCL_Command_hzd_impl_800310D0(void)
{
    ...
    map->field_8_hzd = Map_HZD_Load_80030F38(
                                    GCL_GetNextParamValue_80020AD4(),
                                    GCL_GetNextParamValue_80020AD4(),
                                    map->field_0_map_index_bit,
                                    d1, d2);

    if (GCL_GetParam_80020968('l')) // lit
    {
        map->field_C_lit_file = GV_GetCache_8001538C(
                GV_CacheID_800152DC(GCL_GetNextParamValue_80020AD4(), 0x6C));
    }
    else 
    {
